...
| 1.2.a Migration: snapshot columns on `visit_households` | ✅ | _pending_ | ✅ Columns exist; 108 historical rows backfilled in dev DB; NULL count = 0; pivot columns exposed via `withPivot()` on both relationships |
...
| 2026-04-29 | 1.2.a | (not in spec) | Snapshot columns are nullable, not NOT NULL DEFAULT 0 | Reviewer endorsed: NULL fails loud at read time if a row is created without a snapshot, while 0 silently corrupts SUMs. **Open requirement carried into 1.2.c**: report queries must use `COALESCE(vh.household_size, 0)` and log a warning on NULL pivot rows; otherwise a single bad row poisons the whole report total. |
