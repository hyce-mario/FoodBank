...
| 1.1.c.2 `EventDayController::reorder` transaction + version check | ✅ | _pending_ | ✅ Reorder runs inside `DB::transaction` + `lockForUpdate`; NULL-stage allows two-row swaps; per-move `updated_at` enforces optimistic versioning; concurrent reorders that lose the version race get 409 + auto-refetch in the client |
| 1.1.c.3 `VisitMonitorController::reorder` swap to `VisitReorderService` | ⬜ | — | Same race fix applied to admin monitor endpoint; reuses the service from 1.1.c.2 |
