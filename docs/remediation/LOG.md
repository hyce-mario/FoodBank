...
| 1.2.c Switch `ReportAnalyticsService` to read from snapshot | ✅ | _pending_ | ✅ overview/overviewTrend/eventPerformance/trends/demographics/exportVisits all read snapshot demographics from `vh.*`; exportHouseholds deliberately stays live (current roster); 4 regression tests pin temporal stability |
