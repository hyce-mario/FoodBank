...
| 1.3.b `CheckInController::store` catch + 422 response | ⬜ | — | Controller surfaces conflict as JSON 422 with `allow_override` + `household_ids`; accepts `force=1` + `override_reason` |
| 1.3.c Override modal in `checkin/index.blade.php` | ⬜ | — | Server-rendered modal shows offending households; reason textarea (required if `allow_override`); re-POSTs with `force=1` |
