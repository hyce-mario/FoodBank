...
@php
    ...
    $exportRoutes = [
        'print' => 'finance.reports.expense-detail.print',
        'pdf'   => 'finance.reports.expense-detail.pdf',
        'csv'   => 'finance.reports.expense-detail.csv',
    ];
...
<form method="GET" action="{{ url()->current() }}"
    ...
    <select name="status"
            class="px-3 py-2 text-sm border border-gray-300 rounded-lg bg-white
                   focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-400 text-gray-700">
        <option value="" @selected(empty($filters['status']))>Completed only</option>
...
<div class="grid grid-cols-3 gap-4 mb-5">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4">
        ...
        @if ($hasCompare && $data['prior_total'] !== null && $data['prior_total'] > 0)
            ...
            {{-- For expenses ▲ is bad — flip the colour semantics --}}
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4">
        <p class="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-1">Top Vendor</p>
        @if ($data['top_source'])
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-gray-100 bg-gray-50/40 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    ...
                    <th class="text-left px-5 py-3">Title / Payee</th>
            ...
            <tbody class="divide-y divide-gray-50">
                        ...
                        <td colspan="5" class="px-5 py-2 text-xs font-bold uppercase tracking-widest text-navy-700 flex items-center gap-2">
                            ...
                            <span class="ml-auto text-gray-500 normal-case font-normal tracking-normal">
                                ...
                                @if ($hasCompare && $cat['delta'] !== null)
                                    {{-- Expenses inverted: down is good, up is bad --}}
                ...
                @if (empty($data['rows']))
                    <tr><td colspan="5" class="px-5 py-10 text-center text-gray-400">No expense transactions match the applied filters.</td></tr>
                @endif

                <tr class="border-t-2 border-gray-300 bg-gray-50">
                    <td colspan="4" class="px-5 py-3 font-extrabold uppercase tracking-wide text-sm text-gray-900">Total Expenses</td>
                    <td class="px-5 py-3 text-right tabular-nums font-extrabold text-base text-red-700">{{ \App\Services\FinanceReportService::usd($data['total']) }}</td>
                </tr>
            </tbody>
        </table>
    </div>
