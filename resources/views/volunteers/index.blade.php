...
<div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
    ...
    {{-- Toolbar — filters + export buttons. Per-page selector moved to
         the bottom of the table so the toolbar stays focused on filters. --}}
    @php
        // Carry the active filters through to the export endpoints so a
    ...
    @endphp
    <div class="flex flex-col md:flex-row md:items-stretch md:justify-between gap-2 px-4 py-3 border-b border-gray-100 bg-gray-50/50">
        <form method="GET" action="{{ route('volunteers.index') }}"
              class="flex flex-col sm:flex-row sm:flex-wrap gap-2 flex-1">
        ...
        {{-- Export buttons — Print / PDF / CSV. Carry active filters via
             query string so a filtered view exports the same subset. --}}
        <div class="flex items-center gap-2 md:flex-shrink-0">
            <a href="{{ route('volunteers.export.print', $exportQuery) }}"
               target="_blank"
               class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold border border-gray-300 text-gray-700 hover:bg-gray-50 rounded-lg transition-colors">
            ...
            <a href="{{ route('volunteers.export.pdf', $exportQuery) }}"
               class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold border border-red-200 text-red-700 hover:bg-red-50 rounded-lg transition-colors">
            ...
            <a href="{{ route('volunteers.export.csv', $exportQuery) }}"
               class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold border border-green-200 text-green-700 hover:bg-green-50 rounded-lg transition-colors">
