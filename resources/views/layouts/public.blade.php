...
<head>
    ...
    @php
        ...
        $logoDataUri    = \App\Services\SettingService::brandingLogoDataUri();
    @endphp
    @if($faviconDataUri)
...
<div class="relative z-10 min-h-screen flex flex-col">
    ...
    <header class="w-full px-6 py-5">
        <div class="max-w-3xl mx-auto flex items-center justify-between">
            {{-- Logo (uploaded brand logo from settings, with the bag-icon fallback) --}}
            <div class="flex items-center gap-2.5">
                @if($logoDataUri)
                    <img src="{{ $logoDataUri }}"
                         alt="{{ config('app.name', 'FoodBank') }}"
                         class="h-10 w-auto max-w-[160px] object-contain flex-shrink-0">
                @else
