...
class UpdateHouseholdRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            // ── Represented households (update/create/detach) ─────────────────
            'represented_households'                   => ['nullable', 'array', 'max:10'],
            'represented_households.*.id'              => ['nullable', 'integer', 'exists:households,id'],
            'represented_households.*._detach'         => ['nullable', 'boolean'],
            'represented_households.*.first_name'      => ['required_without:represented_households.*._detach', 'string', 'max:100'],
            'represented_households.*.last_name'       => ['required_without:represented_households.*._detach', 'string', 'max:100'],
            'represented_households.*.email'           => ['nullable', 'email', 'max:255'],
            'represented_households.*.phone'           => ['nullable', 'string', 'max:20'],
            'represented_households.*.children_count'  => ['nullable', 'integer', 'min:0', 'max:50'],
            'represented_households.*.adults_count'    => ['nullable', 'integer', 'min:0', 'max:50'],
            'represented_households.*.seniors_count'   => ['nullable', 'integer', 'min:0', 'max:50'],
